...
var KTFileManagerList = function () {
    ...
    const initDatatable = () => {
        ...
        tableRows.forEach(row => {
            ...
            const realDate = moment(dateCol.innerHTML, "YYYY MM DD, LT").format();
...
//                   <img src="${hostUrl}/resources/demo1/assets/media/illustrations/sketchy-1/5.png" class="mw-400px" />
//                    <div class="fs-1 fw-bolder text-dark">등록된 파일이 없습니다.</div>
            'language': {
                emptyTable: `<div class="d-flex flex-column flex-center">
                    <div class="fs-6">첨부된 파일이 없습니다.</div>
                </div>`
            }
        };
...
//                    <img src="${hostUrl}/resources/demo1/assets/media/illustrations/sketchy-1/5.png" class="mw-400px" />
//                    <div class="fs-1 fw-bolder text-dark mb-4">등록된 파일이 없습니다.</div>
            'language': {
                emptyTable: `<div class="d-flex flex-column flex-center">
                    <div class="fs-6">첨부된 파일이 없습니다.</div>
                </div>`
    ...
    const renameCallback = (e) => {
        ...
        if (table.querySelectorAll('#kt_file_manager_rename_input').length > 0) {
            Swal.fire({
                text: "저장되지 않았습니다. 현재 항목을 저장하거나 취소하세요.",
                icon: "warning",
                buttonsStyling: false,
                confirmButtonText: "확인",
                customClass: {
    ...
    const initDropzone = () => {
        ...
        // 더 이상 아무것도 업로드되지 않을 때 전체 진행률 표시줄 숨기기
 ...
 //       counter.innerText = datatable.rows().count() + ' items';
    ...
    return {
        init: function () {
            ...
            initTemplates(); // 템플릿 요소 할당 하기
            initDatatable(); // 데이터테이블 초기화
            initToggleToolbar(); // 토글 툴바 : 선택한 항목 삭제
            handleSearchDatatable(); // 검색
            handleDeleteRows(); // 파일 > 더보기 > 삭제
            handleNewFolder(); // 새폴더
            initDropzone(); // Dropzone
            initCopyLink(); // 링크 복사
            handleRename(); // 이름 바꾸기
            handleMoveToFolder(); // 폴더 이동
            countTotalItems(); // 아이템 갯수
            KTMenu.createInstances();
        }
    }
}();
